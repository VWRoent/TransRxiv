...
追加/変更（本更新）:
- 設定保存：Fetch 実行時の設定を ./setting/setting.txt に上書き保存し、./log/setting/yyyy-MM-dd_hh-mm.log にも保存
- ログ保存：
    ・正常終了時: Logs の内容を ./log/fetch/yyyy-MM-dd_hh-mm.log に保存（ファイル名の時刻は Fetch 開始時刻）
    ・中断時: 中断ボタン押下時刻で ./log/pause/yyyy-MM-dd_hh-mm.log に保存
- 日ナビ追加に加え、ナビボタン（◀年 ◀月 ◀日 ▶日 ▶月 ▶年）を「横方向だけ」大きく、固定幅で整列
- 期間指定（Day/Week/Month/Year）＆ Stop Now / Stop After This Day は前回実装を踏襲
- ディレクトリ構成、キーワードフィルタ、カテゴリのスラグ化、各種 index/log 更新は前回の通り
"""
...
def update_month_log(base_dir: Path, date: str, collected: int):
    """./log/YYYY/MM/all_month.html に追記"""
    y, m, _ = date.split("-")
    ...
    # 相対リンク: log/YYYY/MM/all_month.html → date/YYYY-MM-DD/date.html
...
def update_year_log(base_dir: Path, date: str):
    """./log/year/all_year.html に YYYY-MM 行を追記（重複防止）"""
    y, m, _ = date.split("-")
    ...
    ym = f"{y}-{m}"
    # 相対リンク: log/year/all_year.html → log/YYYY/MM/all_month.html
...
def process_record(base_dir: Path, rec: dict, total: int, index: int,
    ...
    # 個別HTML: ./date/YYYY-MM-DD/<cat_slug>/<doidate>/<doino>.html
    ...
    # ===== ./date/YYYY-MM-DD/<cat_slug>/category.html =====
    ...
    # ===== ./category/<cat_slug>/category.html （全日付集約） =====
    ...
    # ./category/<cat_slug>/category.html → ../../date/YYYY-MM-DD/<cat_slug>/<doidate>/<doino}.html
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.start_ts_str: Optional[str] = None  # yyyy-MM-dd_hh-mm（Fetch開始）
        self.pause_ts_str: Optional[str] = None  # yyyy-MM-dd_hh-mm（中断ボタン押下）

        frm = ttk.Frame(self, padding=10)
        frm.pack(fill="x")

        # スタイル（ナビボタンは横幅のみUP、固定幅・高さはほぼ既定）
        self.style = ttk.Style(self)
        self.style.configure("NavWide.TButton", padding=(18, 2))  # 横だけ広め、縦は最小限
    ...
    def shift_date(self, years=0, months=0, days=0):
        ...
        # 月/年を動かす場合は day を維持し、存在しなければ末日でクリップ
        if months or years:
    ...
    def refresh_category_list(self):
        ...
        if cats_dir.exists():
            for p in sorted(cats_dir.iterdir()):
                if p.is_dir():
                    items.append(p.name)  # slug フォルダ名
        ...
        def worker():
            try:
                for di, day_str in enumerate(dates, start=1):
                    ...
                    # 当日分の索引・ログを更新（部分完了でも processed を記録）
                ...
                # ログ保存：中断かどうかで保存先を切替
                if self.stop_now_event.is_set() or self.stop_after_day_event.is_set():
