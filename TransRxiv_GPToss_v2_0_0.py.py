...
今回の更新:
- HTTPS(doi等) をビューア内で表示: requests→<base>注入→target=_self で load_html。失敗時は load_website()→外部ブラウザ。
- ローカルHTMLは <base> 注入＋ target="_blank"→"_self"（メモリ内のみ）で遷移可能に。
- 住所バー更新＆Homeボタン（最初に開いたページへ）。
- 421 対策: requests に UA + Connection: close。
- set_zoom が無い環境では CSS zoom を自動注入。
"""
...
REQ_HEADERS = {
    "User-Agent": "biorxiv-gui/1.1 (+https://example.local)",
    "Connection": "close",
}
...
def parse_cursor_from_url(url: str) -> Tuple[int, str, str]:
    ...
    url0 = url + ("0" if url.endswith("/") else "/0")
...
def translate_title_abstract_ja(title_en: str, abstract_en: str,
    ...
    payload = {"model": lm_model, "messages": messages, "temperature": LM_TEMPERATURE}
...
class FlashDisplay:
    ...
    def open(self):
        ...
        try: self.top.attributes("-topmost", True)
        except Exception: pass

        self.title_lbl = tk.Label(self.top, text="", fg="#FFF", bg="#000",
                                  font=("Helvetica", 24, "bold"), wraplength=920, justify="left")
        self.title_lbl.pack(fill="x", padx=20, pady=(20, 10))

        self.info_lbl = tk.Label(self.top, text="", fg="#EEE", bg="#000",
                                 font=("Helvetica", 18), wraplength=920, justify="left")
        self.info_lbl.pack(fill="x", padx=20, pady=(0, 6))

        self.meta_lbl = tk.Label(self.top, text="", fg="#A9A9A9", bg="#000",
    ...
    def on_close(self):
        if self.top and self.top.winfo_exists():
            self.top.destroy()
        self.top = None; self.title_lbl = None; self.info_lbl = None; self.meta_lbl = None

    def show(self, title_ja: str, corr: str, inst: str, authors: str, license_str: str):
        if not (self.top and self.top.winfo_exists()):
            self.open()
        if self.title_lbl: self.title_lbl.config(text=title_ja or "")
        lines_info = [x for x in (corr, inst) if x]
        if self.info_lbl: self.info_lbl.config(text="\n".join(lines_info))
        lines_meta = []
        if authors: lines_meta.append(f"author: {authors}")
        if license_str: lines_meta.append(f"license: {license_str}")
        if self.meta_lbl: self.meta_lbl.config(text="\n".join(lines_meta))

# ===== ViewerWindow =====
def _inject_base(html: str, base_href: str) -> str:
    try:
        if "<head" in html and "<base" not in html:
            html = re.sub(r"(<head[^>]*>)", r"\1" + f'<base href="{base_href}">', html, count=1,
                          flags=re.IGNORECASE | re.DOTALL)
    except Exception:
        pass
    return html

def _rewrite_targets_to_self(html: str) -> str:
    try:
        return re.sub(r'target\s*=\s*"_blank"', 'target="_self"', html, flags=re.IGNORECASE)
    except Exception:
        return html
...
class ViewerWindow(tk.Toplevel):
    def __init__(self, master, base_dir_getter, initial_zoom=1.0, zoom_changed_cb=None, initial_font=""):
        ...
        self.home_target: Optional[Tuple[str, str]] = None
        ...
        # Zoom
        zbar = ttk.Frame(self, padding=(8,0,8,6)); zbar.pack(fill="x")
        ttk.Label(zbar, text="Zoom:").pack(side="left")
        self.zoom_label = ttk.Label(zbar, width=6, anchor="e"); self.zoom_label.pack(side="left", padx=(4,4))
        ttk.Button(zbar, text="-", width=3, command=lambda: self.set_zoom(self.zoom_level - 0.1)).pack(side="left")
        ttk.Button(zbar, text="+", width=3, command=lambda: self.set_zoom(self.zoom_level + 0.1)).pack(side="left", padx=(4,8))
        self.zoom_slider = ttk.Scale(zbar, from_=0.5, to=2.0, orient="horizontal",
                                     command=lambda v: self.set_zoom(_safe_float(v, self.zoom_level)), length=260)
        self.zoom_slider.pack(side="left", padx=(0,8))
        try: self._zoom_slider_cmd = self.zoom_slider.cget("command")
        except Exception: self._zoom_slider_cmd = None
        ttk.Button(zbar, text="Reset 100%", command=lambda: self.set_zoom(1.0)).pack(side="left")

        # Font
        fbar = ttk.Frame(self, padding=(8,0,8,6)); fbar.pack(fill="x")
        ttk.Label(fbar, text="Font:").pack(side="left")
        self.font_var = tk.StringVar(value=self.font_family)
        self.font_cb = ttk.Combobox(fbar, textvariable=self.font_var, state="readonly",
                                    width=32, values=list_system_fonts())
    ...
    def set_zoom(self, level: float):
        ...
        if callable(self.zoom_changed_cb):
            try: self.zoom_changed_cb(level)
            except Exception: pass
    ...
    def _on_link_clicked(self, url: str):
        try: url = (url or "").strip()
        except Exception: return "break"
        if not url: return "break"
        ...
        if self.last_local_dir:
            ...
            try: p = Path(urllib.parse.unquote(url)).resolve()
            except Exception: p = Path(url)
        self.open_local(p)
        return "break"
    ...
    def open_local(self, path: Path, push_history=True):
        ...
        html = read_text(path)
        base = path.parent.as_uri().rstrip("/") + "/"
        html = _inject_base(html, base)
        html = _rewrite_targets_to_self(html)
        if self.font_family:
            html = inject_font_css(html, self.font_family)
        html = self._inject_zoom_css(html)

        self.web.load_html(html)
        self.addr_var.set(str(path))
        self.current = ("file", str(path))
        if push_history and not self.home_target:
            self.set_home("file", str(path))

    def open_url(self, url: str, push_history=True):
        if not HAS_TKINTERWEB or not getattr(self, "web", None):
            _fallback_open(url); self.addr_var.set(url); self.current = ("url", url); return

        # 1) requests で取得 → <base> + target=_self → CSS zoom → load_html
        try:
            r = requests.get(url, timeout=20, headers=REQ_HEADERS, allow_redirects=True)
            r.raise_for_status()
            html = r.text
            base = (r.url.rsplit("/", 1)[0] + "/") if "/" in r.url else r.url
            html = _inject_base(html, base)
            html = _rewrite_targets_to_self(html)
            if self.font_family:
                html = inject_font_css(html, self.font_family)
            ...
            # 2) だめなら組み込み loader
            try:
                self.web.load_website(url)
            except Exception as e:
                self.web.load_html(f"<html><body><h3>読み込みに失敗しました</h3><p>{escape(url)}</p><pre>{escape(str(e))}</pre></body></html>")
                return

        self.addr_var.set(url)
        self.current = ("url", url)
        if push_history and not self.home_target:
            self.set_home("url", url)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.style = ttk.Style(self); self.style.configure("NavWide.TButton", padding=(18, 2))
        ...
        navf = ttk.Frame(frm); navf.grid(row=1, column=2, columnspan=6, sticky="w")
        wch = 6
        ...
        # License
        licf = ttk.LabelFrame(self, text="License Filter", padding=8); licf.pack(fill="x", padx=10, pady=(4, 8))
        ...
        # Keyword
        kwf = ttk.LabelFrame(self, text="Keyword Filter", padding=8); kwf.pack(fill="x", padx=10, pady=(0,8))
        ...
        # Actions
        act = ttk.Frame(self, padding=(10,0,10,5)); act.pack(fill="x")
        self.run_btn = ttk.Button(act, text="Fetch & Generate", command=self.on_run); self.run_btn.pack(side="left", padx=(0,8))
        self.stop_now_btn = ttk.Button(act, text="Stop Now", command=self.on_stop_now, state="disabled"); self.stop_now_btn.pack(side="left", padx=(0,8))
        self.stop_after_day_btn = ttk.Button(act, text="Stop After This Day", command=self.on_stop_after_day, state="disabled"); self.stop_after_day_btn.pack(side="left", padx=(0,8))
        self.stop_after_article_btn = ttk.Button(act, text="Stop After This Article", command=self.on_stop_after_article, state="disabled"); self.stop_after_article_btn.pack(side="left", padx=(0,8))
        self.pause_btn = ttk.Button(act, text="Pause", command=self.on_pause, state="disabled"); self.pause_btn.pack(side="left", padx=(0,8))
        self.resume_btn = ttk.Button(act, text="Resume", command=self.on_resume, state="disabled"); self.resume_btn.pack(side="left", padx=(0,8))

        # TTS & Flash
        ttsf = ttk.Frame(self, padding=(10, 0, 10, 5)); ttsf.pack(fill="x")
        ...
        # Progress
        prf = ttk.Frame(self, padding=(10,0,10,5)); prf.pack(fill="x")
        self.progress = ttk.Progressbar(prf, orient="horizontal", mode="determinate", maximum=100)
        self.progress.pack(fill="x", pady=(5,2))
        self.prog_lbl = ttk.Label(prf, text="Progress: -/-"); self.prog_lbl.pack(anchor="w")

        # Logs
        lgf = ttk.LabelFrame(self, text="Logs", padding=8); lgf.pack(fill="both", expand=True, padx=10, pady=5)
        self.log_text = ScrolledText(lgf, height=14, font=("Consolas", 10)); self.log_text.pack(fill="both", expand=True)

        # Viewer controls
        vf = ttk.LabelFrame(self, text="Viewer Controls (別ウィンドウで表示)", padding=8); vf.pack(fill="x", padx=10, pady=6)
        ...
        # Openers
        btm = ttk.LabelFrame(self, text="Open Index (外部ブラウザ/システム)", padding=8); btm.pack(fill="x", padx=10, pady=5)
        ...
        # Footer
        opf = ttk.Frame(self, padding=10); opf.pack(fill="x")
        self.open_btn = ttk.Button(opf, text="Open Output Folder", command=self.open_output_folder, state="normal"); self.open_btn.pack(side="left")
    ...
    def _load_settings_on_start(self):
        try:
            ...
            if set_path.exists():
                ...
                try: vz = float(dic.get("viewer_zoom", "1.0"))
                except Exception: vz = 1.0
    ...
    def ensure_viewer_window(self) -> ViewerWindow:
        if self.viewer_win and self.viewer_win.winfo_exists():
            try: self.viewer_win.deiconify(); self.viewer_win.lift()
            except Exception: pass
            ...
            if txt:
                for line in txt.splitlines():
                    if line.startswith("viewer_font="):
                        lines.append(f"viewer_font={self.viewer_font}"); found = True
                    else:
                        lines.append(line)
            if not found: lines.append(f"viewer_font={self.viewer_font}")
            write_text(set_path, "\n".join(lines))
        except Exception as e:
            ...
            if txt:
                for line in txt.splitlines():
                    if line.startswith("viewer_zoom="):
                        lines.append(f"viewer_zoom={level}"); found = True
                    else:
                        lines.append(line)
            if not found: lines.append(f"viewer_zoom={level}")
            write_text(set_path, "\n".join(lines))
        except Exception as e:
    ...
    def refresh_category_list_for_viewer(self):
        ...
        if items: self.cat_var.set(items[0])
    ...
    def speak_title_async(self, base_dir: Path, text: str):
        def play_with_pygame(path: Path) -> bool:
            ...
            try:
                ...
                self.log(f"[TTS] pygame 再生失敗: {e}"); return False

        def runner():
            try:
                from gtts import gTTS
            except Exception as e:
                self.log(f"[TTS] gTTS が利用できません: {e}"); return
            mp3_dir = base_dir / "log" / "tts"; mp3_dir.mkdir(parents=True, exist_ok=True)
            ts = time.strftime("%Y%m%d_%H%M%S", time.localtime()); mp3_path = mp3_dir / f"{ts}.mp3"
            try:
                gTTS(text=text or "", lang="ja").save(str(mp3_path))
            except Exception as e:
                self.log(f"[TTS] 生成失敗: {e}"); return
            if play_with_pygame(mp3_path): return
            try:
                from playsound import playsound; playsound(str(mp3_path)); return
            except Exception as e:
                self.log(f"[TTS] playsound 再生失敗: {e}")
            try:
                if sys.platform.startswith("win"): os.system(f'start /min "" "{mp3_path}"')
                elif sys.platform == "darwin": os.system(f'afplay "{mp3_path}"')
                else: os.system(f'xdg-open "{mp3_path}"')
            except Exception as e:
    ...
    def open_daily_report(self):
        ...
        vw.open_local(p); vw.set_home("file", str(p))
    ...
    def open_category_report(self):
        ...
        if not slug:
            messagebox.showwarning("カテゴリ未選択", "カテゴリを選択してください。"); return
    ...
    def open_by_doino(self):
        ...
        if not doino:
            messagebox.showwarning("doino 未入力", "doino を入力してください。"); return
        base_dir = Path(self.base_dir_var.get()).resolve()
        cat = load_catalog(base_dir)
        hits = [it for it in cat.get("items", []) if str(it.get("doino")) == doino]
        if not hits:
            messagebox.showinfo("見つかりません", f"doino={doino} は catalog.json に見つかりません。"); return
    ...
    def choose_from_candidates(self, items: List[Dict[str, Any]]):
        dlg = tk.Toplevel(self); dlg.title("候補を選択"); dlg.geometry("760x420")
    ...
    def open_random(self):
        ...
        items = cat.get("items", [])
        if not items:
            messagebox.showinfo("catalog.json 空", "catalog.json に記事がありません。Fetch 後にお試しください。"); return
    ...
    def open_selected_category_index(self):
        ...
        if not slug:
            messagebox.showwarning("No category", "カテゴリが見つかりません。Refresh を試してください。"); return
    ...
    def open_selected_date_index(self):
        ...
        if not re.match(r"^\d{4}-\d{2}-\d{2}$", date):
            messagebox.showerror("Invalid date", "日付は YYYY-MM-DD の形式で入力してください。"); return
    ...
    def on_run(self):
        ...
        self.log_buffer = []; self.start_ts_str = self.ts_now_str(); self.pause_ts_str = None
        self.save_settings_files(base_dir, self.start_ts_str)

        self.set_running(True)
        self.stop_now_event.clear(); self.stop_after_day_event.clear(); self.stop_after_article_event.clear(); self.pause_request_event.clear()
        ...
        if kws: self.log(f"[FILTER] keywords={kws} mode={mode}")
        ...
        try:
            ...
            messagebox.showerror("Invalid date", "日付の解析に失敗しました。"); self.set_running(False); return
        ...
        def matches_license(record: dict) -> bool:
            ...
            if lic_preset and lic_preset != "any" and lic != lic_preset: return False
            if require_cc and not is_cc_license_ok(lic): return False
            if ex_by and "by" in lic: return False
            if ex_nc and "nc" in lic: return False
            if ex_nd and "nd" in lic: return False
            if ex_sa and "sa" in lic: return False
            return True
            ...
            try:
                ...
                for di, day_str in enumerate(dates, start=1):
                    if terminate_all or self.stop_now_event.is_set():
                        self.log("[INFO] Stop Now: 期間処理を中断します。"); break
                    ...
                    if self.stop_now_event.is_set():
                        self.log("[INFO] Stop Now: 当日処理を開始前に中断。"); break
                    ...
                    if not records:
                        ...
                        if self.stop_after_day_event.is_set():
                            self.log("[INFO] Stop After This Day: 当日が空のためそのまま終了します。"); break
                        continue
                    ...
                    filtered = kw_filtered; total = len(filtered)
                    ...
                    if total == 0:
                        ...
                        update_year_log(base_dir, day_str)
                        if self.stop_after_day_event.is_set():
                            self.log("[INFO] Stop After This Day: 当日の処理完了（ゼロ件）。終了します。"); break
                        continue
                    ...
                    for i, rec in enumerate(filtered, start=1):
                        if self.stop_now_event.is_set():
                            self.log("[INFO] Stop Now: 当日処理を中断します。"); terminate_all = True; break
                        try:
                        ...
                        if self.stop_after_article_event.is_set():
                            self.log("[INFO] Stop After This Article: 現在の論文処理完了後に停止します。"); terminate_all = True; break
